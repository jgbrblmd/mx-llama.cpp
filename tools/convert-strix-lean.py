...
def quantize_job(job):
    ...
    # Build a byte shape that recovers the original tensor dimensions after quantization.
    # The writer reverses dims on write (ti.shape[n_dims-1-j]), so we pass reversed
    # leading dims plus the first dim converted to bytes.  After quant_shape_from_byte_shape
    # converts the last byte back to elements and the writer reverses again, the file ends
    # up with forward dims matching C++.  This generalises the original 2-D formula
    # [ne1, ne0*tsz//blk] to arbitrary dimensionality.
    # The first dim must satisfy orig[0]*tsz % blk == 0 for a clean byte/element round-trip;
    # otherwise the tensor cannot be cleanly quantised and is kept as F32.
    orig = list(map(int, shape))
    if orig[0] * tsz % blk != 0:
        return None  # signal: keep this tensor as F32 instead of quantising
    byte_shape = list(reversed(orig[1:])) + [orig[0] * tsz // blk]
    return name, quantized, byte_shape, raw_dtype
...
for name, shape, should_quantize, qtype in tensors_info:
    if not should_quantize or qtype == 'F32' or name not in results:
        ...
        # Also covers tensors that could not be cleanly quantised (orig[0]*tsz % blk != 0).
        data = read_source_data(src_by_name[name])
