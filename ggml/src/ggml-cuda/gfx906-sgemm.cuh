...
// GFX906 (MI50/MI60/Radeon VII) custom SGEMM kernel, up to 2x faster than
// rocBLAS for small matrices (M <= 128, N <= 2048, K <= 8192). Ported from
// iacopPBK/llama.cpp-gfx906 (ggml/src/ggml-cuda/gfx906/matmul/sgemm.cuh).
//
// Key optimization: 2-way unrolled inner loop for better instruction-level
// parallelism on GFX906's wave64 execution model.

#ifdef GGML_USE_HIP

#define GFX906_SGEMM_M_TILE 32
#define GFX906_SGEMM_N_TILE 32
#define GFX906_SGEMM_K_TILE 64
...
void gfx906_sgemm_tiled_fast(
    ...
    const int K)
{
    __shared__ float As[GFX906_SGEMM_K_TILE][GFX906_SGEMM_M_TILE + 1];
    __shared__ float Bs[GFX906_SGEMM_K_TILE][GFX906_SGEMM_N_TILE + 1];
    ...
    const int gm_base = bx * GFX906_SGEMM_M_TILE;
    const int gn_base = by * GFX906_SGEMM_N_TILE;

    float acc00 = 0.0f, acc01 = 0.0f, acc10 = 0.0f, acc11 = 0.0f;

    for (int kt = 0; kt < K; kt += GFX906_SGEMM_K_TILE) {
        #pragma unroll
        for (int i = 0; i < 8; i++) {
            int idx = tid * 8 + i;
            int m = idx % GFX906_SGEMM_M_TILE;
            int k = idx / GFX906_SGEMM_M_TILE;
        ...
        for (int i = 0; i < 8; i++) {
            int idx = tid * 8 + i;
            int k = idx % GFX906_SGEMM_K_TILE;
            int n = idx / GFX906_SGEMM_K_TILE;
        ...
        #pragma unroll
        for (int k = 0; k < GFX906_SGEMM_K_TILE; k += 2) {
    ...
    const int stride_C)
{
    ...
    const int num_k_tiles = (K + GFX906_SGEMM_K_TILE - 1) / GFX906_SGEMM_K_TILE;

    for (int kt = 0; kt < num_k_tiles; kt++) {
        const int k_base = kt * GFX906_SGEMM_K_TILE;
...
// Dispatch function - returns true if handled by custom kernel, false if the
// shape falls outside the tuned bounds (caller should fall through to its
// normal dispatch chain in that case). Bounds tuned via benchmarking against
// rocBLAS on MI50 by the upstream fork; kept as-is here.
static bool gfx906_sgemm_custom_dispatch(
    ...
    const int grid_m = (M + GFX906_SGEMM_M_TILE - 1) / GFX906_SGEMM_M_TILE;
    const int grid_n = (N + GFX906_SGEMM_N_TILE - 1) / GFX906_SGEMM_N_TILE;

    dim3 grid(grid_m, grid_n);
    dim3 block(256);

    const bool aligned = (M % GFX906_SGEMM_M_TILE == 0) &&
                         (N % GFX906_SGEMM_N_TILE == 0) &&
                         (K % GFX906_SGEMM_K_TILE == 0);
