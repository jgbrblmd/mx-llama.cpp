...
void quantize_row_nvfp4_e8m0(const float * GGML_RESTRICT x, void * GGML_RESTRICT y, int64_t k) {
    quantize_row_nvfp4_e8m0_ref(x, (block_nvfp4_e8m0 *) y, k);
}
...
void ggml_vec_dot_nvfp4_e8m0_q8_0_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int ib = 0; ib < nb; ++ib) {
        for (int s_idx = 0; s_idx < 4; ++s_idx) {
            const float d = ggml_nvfp4_e8m0_to_fp32(x[ib].d[s_idx]);
            const int q8_block = s_idx / 2;
            const int q8_off   = (s_idx % 2) * QK_NVFP4_SUB;
            const float dy = GGML_CPU_FP16_TO_FP32(y[2*ib + q8_block].d);
...
// ROCmFP3: 32 weights / block, 12 bytes packed 3-bit codes + 2 UE4M3 scale bytes
...
void ggml_vec_dot_rocmfpx_fp3_q8_0(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    static_assert(QK_ROCMFP3 == QK8_0, "QK_ROCMFP3 and QK8_0 must be the same");

    const block_rocmfp3 * GGML_RESTRICT x = vx;
    const block_q8_0    * GGML_RESTRICT y = vy;
